...
import numpy as np
from .barriernet import CLFBarrierNetLayer
...
class MLPBarrierNet(torch.nn.Module):
    def __init__(
        self,
        hidden_layers: int,
        ...
        preprocess_barrier_input_fn: Optional[Callable[[torch.Tensor], torch.Tensor]] = None,
        ...
        """
        super(MLPBarrierNet, self).__init__()
        ...
        self.n_output_dims = n_clf + sum(cbf_rel_degree)
        ...
        # )
        self.policy_layers["output_linear"] = PosLinear(
            self.hidden_layer_width, self.n_output_dims
        )
        # self.policy_layers["output_activation"] = PosLinear()
        ...
        self.barrier_layer = CLFBarrierNetLayer(
    ...
    def forward(self, x: torch.Tensor, x_obs: Optional[torch.Tensor], x_des: Optional[torch.Tensor]):
        ...
        if x_obs is not None and x_des is not None:
            x_in = torch.cat([x, x_obs, x_des], dim=0).to(self.device)
        elif x_obs is not None and x_des is None:
            x_in = torch.cat([x, x_obs], dim=0).to(self.device)
        else:
        ...
        return self.barrier_layer(x.to(self.device), penalty_params)

    def eval_np(self, x: np.ndarray, x_obs: Optional[np.ndarray]= None, x_des: Optional[np.ndarray]= None):
        # Construct the input to the barrier net
        if x_obs is not None and x_des is not None:
            x_in = torch.cat([torch.from_numpy(x), torch.from_numpy(x_obs), torch.from_numpy(x_des)], dim=0).to(self.device)
        elif x_obs is not None and x_des is None:
            x_in = torch.cat([torch.from_numpy(x), torch.from_numpy(x_obs)], dim=0).to(self.device)
        else:
            x_in = torch.from_numpy(x).to(self.device)
        ...
        for epoch in range(n_epochs):
            ...
            for i in epoch_range:
                ...
                for j in range(x_batch.shape[0]):
                    x_in = x_batch[j, :]
                    u_predicted[j, :] = self(x_in, x_obs.squeeze(), x_des.squeeze()).squeeze()
