...
class ClassKNN(torch.nn.Module):
    def __init__(
        ...
        # Create a monotonic neural network for each degree of each barrier function
        self.classk = []

        #Includes obstacle position encoding
        self.mono1 = MonotonicNN(self.n_state_dims+2+1, [hidden_layer_width]*hidden_layers, nb_steps=50, dev=self.device)
        self.mono2 = MonotonicNN(self.n_state_dims+2+1, [hidden_layer_width]*hidden_layers, nb_steps=50, dev=self.device)
    ...
    def _barrier_loss(self, u_train, cbf_params,batch_size):
        ...
        loss += beta1*torch.sum(torch.relu(-(LgLfb * u_train + b_cbf)))
        # constraint satisfaction
        loss += beta2*torch.sum(torch.relu(torch.tanh(LgLfb * u_train + b_cbf)))
        # violation
    ...
    def clone(
        ...
        for epoch in range(n_epochs):
            ...
            for i in epoch_range:
                ...
                # CBF Loss
                loss += self._barrier_loss(u_expert_batch, cbf_params, batch_size)
