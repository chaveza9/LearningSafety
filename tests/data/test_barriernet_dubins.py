...
def define_clf_filter():
    ...
    clf_objective = cp.sum(cp.multiply(r_vals, cp.square(u)))+cp.sum(cp.multiply([30,30], cp.square(delta)))
    # Input constraints
    input_constraint = []
...
def construct_clf_cbf(parameters: torch.Tensor, clf_layer, x, x_obstacle, x_goal)->torch.Tensor:
    ...
    q_vals_clf = torch.tensor([2.0,3])
    # Change states to tensors

    x = torch.from_numpy(x).to(device)
    x_state= torch.zeros(n_states+1).to(device)
    x_state[:2] = x[:2]
    x_state[2] = torch.tensor(0.5)
    x_state[n_states] = x[2]# add
    x = x_state
    # Compute CBF values
